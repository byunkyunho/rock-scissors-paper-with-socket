...
pg.display.set_caption("가위바위보 온라인")
pg.init()
...
def set_game():
    global start_game_om, connect_error, connect_socket, connect_socket_sleep, reconnect_om, connect, mouse_y, mouse_x, my_rsp, left_time, end, back_om
    start_game_om = False
...
def connect_():
    global connect_socket, connect_socket_sleep, connect_error, reconnect_om, client_socket, start_time
    if connect_socket_sleep:
        time.sleep(0.5)
        connect_socket_sleep = False
        start_time = time.time()
    if not connect_socket and not connect_error:
        try:
...
while running:
    ...
    while game and running:
        ...
        if  connect:
            ...
            draw_text("{}".format(5 - int(time.time() - start_time)),90, 230, -6)
        if connect_error:
            draw_text("연결오류", 100, 50, 100)
            if check_on(mouse_x, mouse_y, 177, 318, 50, 150):
                reconnect_om = True
                draw_text("재접속", 60, 167, 313)
            else:
                draw_text("재접속", 50, 177, 318)
                reconnect_om = False
        else:
            d_rsp()
            if not connect:
